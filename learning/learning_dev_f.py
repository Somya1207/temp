...
class AbstractLearningDevF(AbstractLearning):
    ...
    def fit_from_tensors(self, x_train, y_train, z_train, x_dev, z_dev, y_dev,
        ...
        # optimizer_g = CustomSGD(self.g.parameters(), lr=self.g_optimizer.param_groups[0]['lr'])
        # optimizer_f = CustomSGD(self.f.parameters(), lr=self.f_optimizer.param_groups[0]['lr'])
        optimizer_g = CustomSGD(self.g.parameters(), lr=0.3)
        optimizer_f = CustomSGD(self.f.parameters(), lr=0.3)

# Now pass the optimizer instance to the scheduler
        ...
        for i in range(self.max_num_iterations):
            # self.update_params_iter(i, x_train, z_train, y_train,scheduler_g,optimizer_g,scheduler_f,optimizer_f)
...
class SGDLearningDevF(AbstractLearningDevF):
    ...
    def update_params_iter(self, iteration, x, z, y):
        ...
        # scheduler_g.step()
        # scheduler_f.step()
        # wandb.log({"epoch":iteration,"lr":scheduler_g._last_lr[0]})
        # wandb.log({"epoch":iteration,"lr":scheduler_f._last_lr[0]})
